...
public:
...
#ifdef isunicode
	static void vAppendFormattedString(qstring &appendTo, const qstring *pFormat, va_list pArgList);
	static qstring *newStringFromFormat(const qoschar *pFormat, ...); // Create a new qstring instance based on a formatted string
	static qstring *newStringFromFormat(const qstring *pFormat, ...); // Create a new qstring instance based on a formatted string
#endif
	...
	qstring &appendFormattedString(const qstring *pFormat, ...); // Append a formatted string to our string
#endif
	...
	const qchar &operator[](qlong pIndex) const; // Get a pointer to character at a specific location
