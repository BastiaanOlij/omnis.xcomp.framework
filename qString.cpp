...
void qstring::vAppendFormattedString(qstring &appendTo, qstring &pFormat,
                                     va_list pArgList) {
  qlong lvPos = 0;
      ...
      if ((lvChar == '%') || (lvChar == ' ') || (lvPos == lvLen)) {
        ...
        switch (lvFormat[lvFormat.length() - 1]) {
          /*					case 'i': {
                                                          char	lvBuffer[256];
                                                          qlong	lvVal =
             va_arg(pArgList, qlong); sprintf(lvBuffer, lvFormat.c_str(),
             lvVal); // cheat, use sprintf.. appendTo += lvBuffer;
                                                  }; break;
                                                  case 'f': {
                                                          char	lvBuffer[256];
                                                          double	lvVal =
             va_arg(pArgList, double); sprintf(lvBuffer, lvFormat.c_str(),
             lvVal); // cheat, use sprintf.. appendTo += lvBuffer;
                                                  }; break;
           */
